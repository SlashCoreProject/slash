...
const char * const BITCOIN_CONF_FILENAME = "slash.conf";
const char * const BITCOIN_PID_FILENAME = "slashd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "slash" is a composite category enabling all Slash-related debug output
            if(ptrCategory->count(string("slash"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\SlashCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\SlashCore
    // Mac: ~/Library/Application Support/SlashCore
    // Unix: ~/.slashcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "SlashCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/SlashCore";
#else
    // Unix
    return pathRet / ".slashcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty slash.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override slash.conf
