 ...
 *
 *   // network
 *   CSlashNetwork lvl, network-type, network-status, network-error, milestone-status*
...
private:
    ...
    // Current OBJECT STATUS (see http://govman.slash.org/index.php/Documentation_:_Status_Field)
    int nStatusID;
...
// // root node
class CSlashNetwork : public CGovernanceObject
{
private:
    std::string strName;
    std::string strURL;
...
// // can be under: SlashNetwork
// //   -- signature requirements : Key1(User)
// class CSlashNetworkVariable : public CGovernanceObject
// {
// private:
